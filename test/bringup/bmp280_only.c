/*
 * Bring-up — BMP280 driver verification, no network.
 *
 * Inits I²C0 on GP8/GP9, scans the bus, initialises the BMP280, then prints
 * temp/pressure every second to the USB-serial console. No Wi-Fi, no MQTT —
 * fastest path to "does the sensor work". (The BMP280 has no humidity sensor.)
 */
#include "FreeRTOS.h"
#include "task.h"
...
{
    (void)xTask;
    printf("[bmp280] STACK OVERFLOW in '%s'\n", pcTaskName ? pcTaskName : "?");
    for (;;) { tight_loop_contents(); }
}
void vApplicationMallocFailedHook(void);
void vApplicationMallocFailedHook(void)
{
    printf("[bmp280] malloc failed\n");
    for (;;) { tight_loop_contents(); }
}
...
static void i2c_bus_init(void)
{
    ...
    printf("[bmp280] I²C0 init on SDA=GP%d SCL=GP%d @ %u Hz\n",
           BOARD_I2C_SDA_PIN, BOARD_I2C_SCL_PIN, (unsigned)BOARD_I2C_FREQ_HZ);
}

static void i2c_scan(void)
{
    printf("[bmp280] I²C scan:");
    int count = 0;
...
static void bmp280_task(void *arg)
{
    ...
    printf("\n[bmp280] driver bring-up — pico-sdk + bmp280.c, no network\n");

    i2c_bus_init();
    i2c_scan();

    Bmp280 dev;
    err_t e = bmp280_init(&dev, BOARD_I2C_INST, BOARD_BMP280_ADDR);
    if (e != ERR_OK) {
        for (;;) {
            printf("[bmp280] init FAILED rc=%d (addr 0x%02x). "
                   "Check wiring: VCC=3V3, GND, SDA=GP8, SCL=GP9.\n",
                   (int)e, BOARD_BMP280_ADDR);
            vTaskDelay(pdMS_TO_TICKS(3000));
        }
    }
    printf("[bmp280] init OK at addr 0x%02x\n", BOARD_BMP280_ADDR);

    for (uint32_t n = 0; ; n++) {
        Bmp280Sample s;
        e = bmp280_read_sample(&dev, &s);
        if (e != ERR_OK) {
            printf("[bmp280] sample %u FAILED rc=%d\n", (unsigned)n, (int)e);
        } else {
            printf("[bmp280] %4u  T=%6.2f °C   P=%7.2f hPa\n",
                   (unsigned)n,
                   (double)s.temp_c,
...
int main(void)
{
    ...
    xTaskCreate(bmp280_task, "bmp280", 2048, NULL, 2, NULL);
    vTaskStartScheduler();
    for (;;) { tight_loop_contents(); }
}
