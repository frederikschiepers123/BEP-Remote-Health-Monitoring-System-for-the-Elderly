 ...
 * q_radar, q_light).  Each cycle it drains them to the latest sample, stamps
 * the §9.2.1 envelope (ts_us + per-topic seq), encodes via components/json,
 * publishes to rmms/<uuid>/<kind> (QoS 1), and mirrors the value into the
 * OLED shadow state (ui_oled_set_*).
 *
...
/* Store references to identity + per-deployment config.  Call from app_main
 * before creating the task.  The pointed-to structs must outlive the task. */
